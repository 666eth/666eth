...
contract('Revolution', () => {
  ...
  describe('check initialization', () => {
    ...
    it('maxBalance', async () => {
      ...
      assert.equal(a.toString(10), ether(66600000).toString(10))
    })
  ...
  describe('change state', () => {
    ...
    context('provideAccess', () => {
      it('throw on access denied', async () => {
        await assertRevert(
          instance.provideAccess(addr1, AccessRank.Paymode, { from: addr1 })
        )
  ...
  describe('notOnPause', () => {
    ...
    it('throw doInvest', async () => {
      await nextWavePush()
      await assertRevert(
        instance.doInvest([], { from: addr1, value: ether(1) })
      )
    })
  ...
  describe('doInvest(address[3] refs)', () => {
    ...
    it('throw on low msg.value', async () => {
      ...
      await assertRevert(
        instance.doInvest([0, 0, 0], {
          from: addr1,
          value: minValue.minus(100)
        })
      )
    })
    ...
    context('ref system', () => {
      context('1 level', () => {
        ...
        it('check bonus 3.33% for referral', async () => {
          ...
          assert.equal(info[0].toString(10), ether(1.0666).toString(10))
        })
        ...
        it('dont get bonus if referrer1 is zero', async () => {
          let infoB = await instance.investorInfo(addr2)
          assert.equal(infoB[2].toString(10), '0')
          await instance.doInvest([ZERO_ADDRESS], {
            from: addr2,
            value: ether(1)
          })
        ...
        it('event LogNewReferral', async () => {
          ...
          assert.equal(e.args.addr.toLowerCase(), addr2.toLowerCase())
          assert.equal(e.args.when.toString(10), block.timestamp.toString(10))
          assert.equal(e.args.value.toString(10), ether(1.0666).toString(10))
        })
      })
      context('2 level', () => {
        beforeEach(async () => {
        ...
        it('dont get bonus if referrer2 is sender', async () => {
          let infoB = await instance.investorInfo(addr2)
          assert.equal(infoB[2].toString(10), '0')
          await instance.doInvest([addr1, addr2], {
            from: addr2,
            value: ether(1)
          })
          let infoA = await instance.investorInfo(addr2)
        ...
        it('dont get bonus if referrer2 is zero', async () => {
          let infoB = await instance.investorInfo(addr2)
          assert.equal(infoB[2].toString(10), '0')
          await instance.doInvest([addr1, ZERO_ADDRESS], {
            from: addr2,
            value: ether(1)
          })
          let infoA = await instance.investorInfo(ZERO_ADDRESS)
        ...
        it('dont get bonus if referrer2 is referrer1', async () => {
          let infoB = await instance.investorInfo(addr1)
          assert.equal(infoB[2].toString(10), '0')
          await instance.doInvest([addr1, addr1], {
            from: addr2,
            value: ether(1)
          })
          let infoA = await instance.investorInfo(addr1)
        ...
        it('dont get bonus if referrer2 is not investor', async () => {
          ...
          assert.equal(infoB[2].toString(10), '0')
          await instance.doInvest([addr1, addr3], {
            from: addr2,
            value: ether(1)
          })
      ...
      context('3 level', () => {
        ...
        it('dont get bonus if referrer3 is sender', async () => {
          let infoB = await instance.investorInfo(addr3)
          assert.equal(infoB[2].toString(10), '0')
          await instance.doInvest([addr1, addr2, addr3], {
            from: addr3,
            value: ether(1)
          })
          let infoA = await instance.investorInfo(addr3)
        ...
        it('dont get bonus if referrer3 is zero', async () => {
          let infoB = await instance.investorInfo(addr3)
          assert.equal(infoB[2].toString(10), '0')
          await instance.doInvest([addr1, addr2, ZERO_ADDRESS], {
            from: addr3,
            value: ether(1)
          })
          let infoA = await instance.investorInfo(ZERO_ADDRESS)
        ...
        it('dont get bonus if referrer3 is referrer1', async () => {
          let infoB = await instance.investorInfo(addr1)
          assert.equal(infoB[2].toString(10), '0')
          await instance.doInvest([addr1, addr2, addr1], {
            from: addr3,
            value: ether(1)
          })
          let infoA = await instance.investorInfo(addr1)
        ...
        it('dont get bonus if referrer3 is referrer2', async () => {
          let infoB = await instance.investorInfo(addr2)
          assert.equal(infoB[2].toString(10), '0')
          await instance.doInvest([addr1, addr2, addr2], {
            from: addr3,
            value: ether(1)
          })
          let infoA = await instance.investorInfo(addr2)
        ...
        it('dont get bonus if referrer3 is not investor', async () => {
          ...
          assert.equal(infoB[2].toString(10), '0')
          await instance.doInvest([addr1, addr2, addr4], {
            from: addr3,
            value: ether(1)
          })
    ...
    context('commission', () => {
      it('admin', async () => {
        ...
        assert.equal(
          balanceB.plus(ether(0.1)).toString(10),
          balanceA.toString(10)
        )
      })
      it('payer', async () => {
        await instance.setPayerAddr(addr2, { from: owner })
        ...
        let balanceA = await getBalance(a)
        assert.equal(
          balanceB.plus(ether(0.07)).toString(10),
          balanceA.toString(10)
        )
      })
    })
    ...
    it('contract balance', async () => {
      ...
      assert.equal(
        balanceB.plus(ether(0.83)).toString(10),
        balanceA.toString(10)
      )
    })
  ...
  describe('payout()', () => {
    ...
    context('do sending eth', () => {
      context('without ref bonus', () => {
        it('investor balance', async () => {
          ...
          assert.equal(b.plus(ether(0.0666)).toString(10), a.toString(10))
        })
        it('contract balance', async () => {
          await instance.doInvest([], { from: addr1, value: ether(1) })
          ...
          assert.equal(b.minus(ether(0.0666)).toString(10), a.toString(10))
        })
        it('event LogPayDividends', async () => {
          ...
          assert.equal(e.args.addr.toLowerCase(), addr1.toLowerCase())
          assert.equal(e.args.when.toString(10), block.timestamp.toString(10))
          assert.equal(e.args.value.toString(10), ether(0.0666).toString(10))
        })
      })
    ...
    it('event LogBalanceChanged', async () => {
      ...
      assert.equal(e.args.when.toString(10), block.timestamp.toString(10))
      assert.equal(
        e.args.balance.toString(10),
        ether(0.83 - 0.0666).toString(10)
      )
    })
  ...
  describe('getMyDividends()', () => {
    ...
    it('throw if on pause', async () => {
      await nextWavePush()
      await assertRevert(
        instance.doInvest([], { from: addr2, value: ether(1) })
      )
      await assertRevert(instance.getMyDividends({ from: addr2 }))
    })
    ...
    it('set investor payment time to latest push payment if less than it - 24h', async () => {
      ...
      // only once day
      assert.equal(
        bb.plus(ether(0.0666)).toString(10),
        ba.plus(latestGasUsed() * gasPrice).toString(10)
      )
    })
    ...
    it('dividents after 5 days', async () => {
      ...
      assert.equal(
        bb.plus(ether(5 * 0.0666)).toString(10),
        ba.plus(latestGasUsed() * gasPrice).toString(10)
      )
    })
    ...
    context('send eth', () => {
      context('without bonus', () => {
        ...
        it('contract balance', async () => {
          ...
          assert.equal(bb.minus(ether(0.0666)).toString(10), ba.toString(10))
        })
        it('event LogPayDividends', async () => {
          await instance.getMyDividends({ from: addr1 })
          let block = latestBlock()
      ...
      context('with bonus', () => {
        ...
        it('investor balance', async () => {
          ...
          let ba = await getBalance(addr1)
          assert.equal(
            bb.plus(ether(0.0666 + 0.03)).toString(10),
            ba.plus(cost).toString(10)
          )
        })
        it('contract balance', async () => {
          ...
          let ba = await getBalance(instance.address)
          assert.equal(
            bb.minus(ether(0.0666 + 0.03)).toString(10),
            ba.toString(10)
          )
        })
        it('check info', async () => {
    ...
    it('event LogBalanceChanged', async () => {
      ...
      assert.equal(
        e.args.balance.toString(10),
        ether(0.83 - 2 * 0.0666).toString(10)
      )
    })
  })
